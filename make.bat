...
SET MANAGE=.\venv\Scripts\python classwood/manage.py
SET ENV_FILE=.env
...
:genkey
    IF NOT EXIST .env (
        ...
        .\venv\Scripts\%PYTHON_EXE% -c "from django.core.management.utils import get_random_secret_key;key = get_random_secret_key();print(f'DJANGO_SECRET_KEY={key}\nDEVELOPMENT=True')" -> .env
    ) ELSE (
